...
import { sendMessage, formatRegularMessage, formatEmbedMessages} from './discordApiClient.js'
...
function spawnTailProcess() {
    ...
    try {
        ...
        let tail = new Tail(process.env.FACTORIO_LOG_LOCATION);
...
function sliceMessage(line, token) {
    try { return line.slice(line.indexOf(token) + token.length) }
    catch (error) {
...
function formatAndSendMessage(line) {
    var result = "";
    if (line.includes(TOKENS.PLAYER_CHAT)) {
        ...
        result = formatRegularMessage("***" + sliceMessage(line, TOKENS.BOLD) + "***");
        sendMessage(process.env.DISCORD_CHANNEL_ID_CHAT, result)
    } else if (line.includes(TOKENS.PLAYER_JOIN)) {
        result = formatRegularMessage("***" + sliceMessage(line, TOKENS.PLAYER_JOIN) + "*** has landed. ╚(ಠ_ಠ)=┐");
        sendMessage(process.env.DISCORD_CHANNEL_ID_CHAT, result)
    } else if (line.includes(TOKENS.PLAYER_LEAVE)) {
        result = formatRegularMessage("***" + sliceMessage(line, TOKENS.PLAYER_LEAVE) + "*** left the game. ༼ つ ಥ_ಥ ༽つ");
        sendMessage(process.env.DISCORD_CHANNEL_ID_CHAT, result)
...
function sanitizeChat(line) {
    try {
        let slicedLine = line.slice(line.indexOf(TOKENS.PLAYER_CHAT) + TOKENS.PLAYER_CHAT.length);
        slicedLine = slicedLine.split(":");
        return slicedLine[0] + ": " + slicedLine[1].replace(/[^a-zA-Z0-9?', ]/gi, '');
    } catch (error) {
